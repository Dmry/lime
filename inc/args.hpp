/*=============================================================================
    Original Copyright (c) 2016 Paul Fultz II
    Adaptation (c) 2019 Daniel Emmery
    args.hpp
...
{
    if constexpr (std::is_same<T, bool>() or std::is_same<T, std::nullptr_t>()) return argument_type::none;
    else if constexpr (is_container<T>()) return argument_type::multiple;
...
struct context
{
    ...
    void show_help(std::string name, std::string description, std::string options_metavar) const
    {
        ...
        for(auto&& arg:arguments)
        {
            ...
            this->show_help_col(arg.get_flags(), help, width, total_width);
        }
    ...
    return [](auto&&, auto&, argument& a)
    {
        ...
        a.add_callback([](const argument& arg)
        {
            if (arg.required and arg.count == 0)
                throw std::runtime_error("Required arg missing: " + arg.get_flags() + ". Use --help to display all options.");
        });
    };
}
